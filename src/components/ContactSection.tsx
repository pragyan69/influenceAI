...
const ContactSection = () => {
  ...
  return (
    <section id="contact" className="py-16 sm:py-24 bg-gradient-to-br from-dark-secondary/50 to-dark-primary/80 relative overflow-hidden">
      {/* Background Effects */}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8 mb-16 sm:mb-20">
          {contactMethods.map((method, index) => {
            ...
            return (
              <div
                key={index}
                className="glass-card rounded-3xl p-6 sm:p-8 text-center hover:transform hover:-translate-y-4 hover:scale-105 transition-all duration-500 shadow-2xl hover:shadow-violet-500/20 border border-white/10 hover:border-violet-500/30"
              >
                <div className={`w-16 h-16 sm:w-20 sm:h-20 bg-gradient-to-r ${method.color} rounded-3xl flex items-center justify-center mb-4 sm:mb-6 mx-auto shadow-lg hover:shadow-xl transition-all duration-300`}>
                  {method.iconType === 'image' ? (
                    <img src={method.iconSrc} alt={method.title} className="w-8 h-8 sm:w-10 sm:h-10" />
                  ) : (
                    <method.icon className="w-8 h-8 sm:w-10 sm:h-10 text-white" />
                  )}
                </div>
          ...
          <div className="glass-card rounded-3xl p-8 sm:p-16 max-w-4xl mx-auto shadow-2xl border border-white/10 hover:border-violet-500/30 transition-all duration-500">
            ...
            <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 justify-center">
              <Button 
                asChild
                size="lg"
                className="w-full sm:w-auto bg-gradient-to-r from-violet-deep to-blue-electric hover:from-violet-600 hover:to-blue-600 transition-all duration-300 text-base sm:text-lg px-8 sm:px-10 py-4 sm:py-6 shadow-xl hover:shadow-2xl hover:scale-105"
              >
                <a 
              ...
              <Button 
                ...
                size="lg"
                variant="outline"
                className="w-full sm:w-auto border-white/30 text-white hover:bg-white/10 hover:border-white/50 text-base sm:text-lg px-8 sm:px-10 py-4 sm:py-6 transition-all duration-300 hover:scale-105"
              >
                <a 
